...
**OpenAI-совместимые шлюзы (OpenRouter и т.п.).** Провайдер `openai` принимает
опциональные `OPENAI_BASE_URL` и `OPENAI_API_KEY` из конфигурации и
пробрасывает их в клиент, поэтому любой OpenAI-совместимый шлюз работает без
изменения кода. Для этого провайдера structured output запрашивается методом
`json_schema` (строгий формат ответа) — он устойчивее к слабым моделям, которые
иначе заворачивают JSON в markdown или игнорируют tool-calling; для остальных
провайдеров используется дефолтный tool-calling. Замечание по эксплуатации:
бесплатные модели шлюзов часто перегружены (HTTP 429) и хуже держат строгие
схемы — на такой случай работает штатная деградация (раздел 4.7): кейс уходит
человеку, а не падает.
...
> **Confidence.** Самооценка per-field confidence у LLM ненадёжна (модели
> плохо калибруют собственную уверенность), а strict-схемы многих провайдеров
> не принимают словарь с произвольными ключами. Поэтому экстрактор не просит
> модель возвращать `field_confidence`: каждому реально извлечённому полю
> присваивается фиксированный приор (0.9 у live, 0.98 у fake), а настоящим
> сигналом достоверности служит детерминированная формула валидатора (раздел
> 4.4): grounding-проверка, полнота и warning-флаги. Так «чистый» кейс не
> уходит в ложную эскалацию только из-за того, что модель не проставила числа.
