...
|---|---|
| комплект неполный | auto `reject` (`INCOMPLETE_PACKAGE`) |
| `DOC_EXPIRED` и нет триггеров 7.3 | auto `reject` (`DOCUMENT_EXPIRED`) — детерминированный проверяемый факт |
| любой триггер 7.3 или `degraded=true` | `escalate` → human_review |
...
- [ ] `SPEC.md` синхронизирован с кодом.
- [ ] Граф реализует все паттерны раздела 4; все узлы раздела 8 существуют и покрыты тестами.
- [ ] Синтетические кейсы проходят с ожидаемым поведением: чистый → авто-одобрение; несовпадение → эскалация; санкции/PEP/high-volume → эскалация; битый документ → graceful-деградация к человеку; неполный комплект → авто-reject.
- [ ] Checkpointer (Postgres в проде, Memory в тестах), HITL-прерывание и возобновление работают, включая рестарт процесса между interrupt и resume.
- [ ] Лимиты итераций и retry/fallback реализованы и протестированы.
- [ ] Audit trail пишется по каждому узлу и отдаётся через API.
- [ ] Eval-метрики раздела 11 посчитаны, числа зафиксированы в README; escalation_recall = 1.0.
- [ ] `docker compose up` поднимает систему одной командой; секреты через `.env`; README (англ.) описывает запуск, архитектуру, трейсы.
- [ ] Всё закоммичено и запушено в remote.
